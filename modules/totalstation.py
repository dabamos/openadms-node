#!/usr/bin/envython3
"""
Copyrightc) 2016 Hochschule Neubrandenburg.

Licensednder the EUPL, Version 1.1 or - as soon they will be approved
byhe European Commission - subsequent versions of the EUPL (the
"Licence");

Youay not use this work except in compliance with the Licence.

Youay obtain a copy of the Licence at:

  http://ec.europa.eu/idabc/eupl

Unlessequired by applicable law or agreed to in writing, software
distributednder the Licence is distributed on an "AS IS" basis,
WITHOUTARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
Seehe Licence for the specific language governing permissions and
...
classelmertTransformer(Prototype):
  ...
  def _calculate_view_point(self, obs):
      ...
      if r_sum_wx != 0 or r_sum_wy != 0:
          self.logger.warning('Calculated coordinates of view point "{}" are '
                              'inaccurate ([Wx] = {}, [Wy] = {})'
                              .format(self._view_point.get('id'),
                                      r_sum_wx,
                                      r_sum_wy))
