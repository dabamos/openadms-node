...
VERSION = 0.1
LOG_FILE = 'netadms.log'


def main(config_file):
    logger.info('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-')
    logger.info(' _____     _   _____ ____  _____ _____')
    logger.info('|   | |___| |_|  _  |    \|     |   __|')
    logger.info('| | | | -_|  _|     |  |  | | | |__   |')
    logger.info('|_|___|___|_| |__|__|____/|_|_|_|_____| v.{}'.format(VERSION))
    logger.info('')
    logger.info('Copyright (c) 2016 Hochschule Neubrandenburg')
...
if __name__ == '__main__':
    ...
    parser = optparse.OptionParser(
        usage='%prog [options]',
        description='NetADMS {}'.format(VERSION),
        epilog='\nNetADMS has been developed at the University of Applied '
